...
def render_hero(hourly_df: pd.DataFrame) -> None:
        ...
        html_block(
            ...
            <div class="hero">
                ...
                <div class="hero-now">
                    ...
                    <div class="hero-meta">{omschrijving} · Gevoel {round_int(now['apparent_temperature'])}°C · 💨 {round_int(now['wind_speed_10m'])} km/u</div>
                </div>
            </div>
            """
        ),
...
def build_news_items(hourly_df: pd.DataFrame, daily_df: pd.DataFrame) -> list[str]:
    now = hourly_df.iloc[0]
    ...
    automatisch = [
        ...
        f"Warmste moment rond {warmste['time'].strftime('%H:%M')} met {round_int(warmste['temperature_2m'])}°C",
        f"Meeste neerslag op {format_dag_kop(natste['time'])}: {natste['precipitation_sum']:.1f} mm bij {nat_omschrijving.lower()}",
        f"Sterkste wind rond {windy['time'].strftime('%H:%M')}: {round_int(windy['wind_speed_10m'])} km/u",
    ]
...
def render_daily(daily_df: pd.DataFrame) -> None:
    cards = "".join(build_daily_card(row) for _, row in daily_df.iterrows())
    st.markdown(
        html_block(
            f"""
            <div class="glass-card">
                <div class="section-title">10-daagse verwachting</div>
...
def render_footer() -> None:
    st.markdown(
        html_block("<div class='footer-note'>Copyright: Branding Totaal</div>"),
        unsafe_allow_html=True,
    )
...
def main() -> None:
    ...
    try:
        hourly_df, daily_df = fetch_weather(LATITUDE, LONGITUDE)

        render_hero(hourly_df)
        render_news_ticker(build_news_items(hourly_df, daily_df))
