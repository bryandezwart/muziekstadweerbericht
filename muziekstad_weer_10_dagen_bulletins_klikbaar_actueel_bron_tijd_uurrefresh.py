...
CSS = dedent(
    ...
    .ticker-label {
        ...
        font-size: 0.92rem;
        animation: nieuwsPulse 3.2s ease-in-out infinite;
    }
    ...
    .section-subtitle, .day-pick-note, .radio-copy-hint {
        color: rgba(255,255,255,0.78);
        font-size: 0.98rem;
        line-height: 1.45;
    }
...
def weer_info(code: int, is_day: int | bool | None = None) -> tuple[str, str]:
    code = int(code)
    if is_day is None or pd.isna(is_day):
        return WEER_CODES_DAG.get(code, ("Onbekend", "🌤️"))
    mapping = WEER_CODES_DAG if int(is_day) == 1 else WEER_CODES_NACHT
...
def get_selected_location_from_state() -> dict:
    query = st.session_state.get("location_query", DEFAULT_LOCATION["name"])
    results = search_locations(query)
    current_label = st.session_state.get("selected_location_label", location_label(DEFAULT_LOCATION))
    for loc in results:
        if location_label(loc) == current_label:
            return loc
...
def render_location_picker() -> None:
    ...
    results = search_locations(search_default)
    ...
    st.text_input(
        "Zoek op land, regio of plaats",
        value=search_default,
        key="location_query",
        placeholder="Zoek op land, regio of plaats",
        label_visibility="collapsed",
    )
    st.markdown("<div class='location-help'>Standaardlocatie: De Bilt, Utrecht, Nederland</div>", unsafe_allow_html=True)
...
@st.cache_data(ttl=3600)
def fetch_weather(lat: float, lon: float) -> tuple[pd.DataFrame, pd.DataFrame, dict]:
    response = requests.get(
    ...
    current = {
        "time": pd.to_datetime(current_raw.get("time")) if current_raw.get("time") else hourly.iloc[0]["time"],
        "temperature_2m": current_raw.get("temperature_2m", hourly.iloc[0]["temperature_2m"]),
        "apparent_temperature": current_raw.get("apparent_temperature", hourly.iloc[0]["apparent_temperature"]),
        "weather_code": current_raw.get("weather_code", hourly.iloc[0]["weather_code"]),
        "is_day": current_raw.get("is_day", hourly.iloc[0].get("is_day", 1)),
        "wind_speed_10m": current_raw.get("wind_speed_10m", hourly.iloc[0]["wind_speed_10m"]),
        "relative_humidity_2m": current_raw.get("relative_humidity_2m", hourly.iloc[0].get("relative_humidity_2m", 0)),
    }
    return hourly, daily, current


def nearest_hour_row(hourly_df: pd.DataFrame, ts: pd.Timestamp) -> pd.Series:
    deltas = (hourly_df["time"] - ts).abs()
    idx = deltas.idxmin()
    return hourly_df.loc[idx]


def enable_hourly_autorefresh() -> None:
    components.html(
        """
        <script>
        (function() {
            ...
            const remaining = Math.max(1000, nextHour - now);
            window.clearTimeout(window.parent.__muziekstadRefreshTimer);
            window.parent.__muziekstadRefreshTimer = window.setTimeout(function() {
...
def render_hero(current: dict, location_name: str) -> None:
    omschrijving, icoon = weer_info(current["weather_code"], current.get("is_day"))
    logo = logo_base64()
    updated_at = current.get("time")
    if isinstance(updated_at, pd.Timestamp) and not pd.isna(updated_at):
        updated_text = updated_at.strftime("%H:%M")
    else:
        updated_text = "onbekend"
    ...
    st.markdown(
        html_block(
            ...
            <div class="hero">
                {left_html}
                <div class="hero-now">
                    <div class="hero-temp">{round_int(current['temperature_2m'])}°C {icoon}</div>
                    <div class="hero-meta">{location_name} · {omschrijving} · Gevoel {round_int(current['apparent_temperature'])}°C · 💨 {round_int(current['wind_speed_10m'])} km/u</div>
                    <div class="hero-submeta">Laatst bijgewerkt: {updated_text} · Bron: Open-Meteo · Automatische verversing: ieder uur</div>
                </div>
            </div>
            """
        ),
...
def build_news_items(current: dict, hourly_df: pd.DataFrame, daily_df: pd.DataFrame, location_name: str) -> list[str]:
    now = current
    ...
    nu_omschrijving, _ = weer_info(now["weather_code"], now.get("is_day"))
...
def render_summary(current: dict, hourly_df: pd.DataFrame, daily_df: pd.DataFrame) -> None:
    warmste = hourly_df.loc[hourly_df["temperature_2m"].idxmax()]
    natste = daily_df.loc[daily_df["precipitation_sum"].idxmax()]
    windy = hourly_df.loc[hourly_df["wind_speed_10m"].idxmax()]
    ...
    warm_omschrijving, _ = weer_info(warmste["weather_code"], warmste.get("is_day"))
    nu_omschrijving, _ = weer_info(current["weather_code"], current.get("is_day"))
    ...
    cards = [
        ...
        summary_card(
            "Komende uren",
            f"Nu {round_int(current['temperature_2m'])}°C",
            f"Op dit moment is het {nu_omschrijving.lower()} en voelt het aan als {round_int(current['apparent_temperature'])}°C. Binnen 12 uur ligt de temperatuur grofweg tussen {round_int(komende['temperature_2m'].min())}°C en {round_int(komende['temperature_2m'].max())}°C.",
        ),
    ]
...
def build_hour_card(row: pd.Series, include_day: bool = False) -> str:
    omschrijving, icoon = weer_info(row["weather_code"], row.get("is_day"))
    return html_block(
        f"""
        <div class="hour-card">
...
def wind_beschrijving(speed: float) -> str:
    speed = float(speed)
    if speed < 15:
        return "zwakke wind"
    if speed < 25:
        return "matige wind"
    if speed < 40:
        return "vrij krachtige wind"
    if speed < 55:
        return "krachtige wind"
    return "harde wind"


def regen_zin(prob: float, mm: float) -> str:
    prob = float(prob)
    mm = float(mm)
    if mm < 0.2 and prob < 20:
        return "Het lijkt waarschijnlijk droog te blijven."
    if mm < 1.0:
        return f"De kans op een spatje of lichte regen ligt rond {round_int(prob)} procent."
    if mm < 4.0:
        return f"Er is kans op regen of een paar buien, met ongeveer {mm:.1f} millimeter neerslag."
    return f"Het wordt een nattere dag met naar verwachting ongeveer {mm:.1f} millimeter neerslag."


def temperatuur_zin(max_temp: float, min_temp: float) -> str:
    max_temp = float(max_temp)
    min_temp = float(min_temp)
    return f"De middagtemperatuur komt uit rond {round_int(max_temp)} graden en in de vroege uren zakt het naar ongeveer {round_int(min_temp)} graden."


def build_bulletins(row: pd.Series, location_name: str) -> tuple[str, str]:
    omschrijving, _ = weer_info(row["weather_code"])
    day_name = DAGNAAM_VOL[row["time"].weekday()].capitalize()
    short = (
        f"{day_name} wordt het in {location_name} {omschrijving.lower()}, met maxima rond {round_int(row['temperature_2m_max'])} graden "
        f"en minima rond {round_int(row['temperature_2m_min'])} graden. "
        f"{regen_zin(row['precipitation_probability_max'], row['precipitation_sum'])} "
        f"De wind is {wind_beschrijving(row['wind_speed_10m_max'])} en kan pieken rond {round_int(row['wind_speed_10m_max'])} kilometer per uur."
    )
    extended = (
        f"Voor {day_name.lower()} ziet het weerbeeld in {location_name} er {omschrijving.lower()} uit. "
        f"{temperatuur_zin(row['temperature_2m_max'], row['temperature_2m_min'])} "
        f"{regen_zin(row['precipitation_probability_max'], row['precipitation_sum'])} "
        f"Verder staat er een {wind_beschrijving(row['wind_speed_10m_max'])} tot ongeveer {round_int(row['wind_speed_10m_max'])} kilometer per uur. "
        f"De zon komt op om {row['sunrise'].strftime('%H:%M')} en gaat onder om {row['sunset'].strftime('%H:%M')}."
    )
    return short, extended


def render_bulletin_picker(daily_df: pd.DataFrame, location_name: str) -> None:
    if "selected_bulletin_day_idx" not in st.session_state:
        st.session_state["selected_bulletin_day_idx"] = 0
    ...
    st.markdown("<div class='day-pick-note'>Klik op een dagknop hieronder om de belangrijkste info plus voorleesteksten te tonen.</div>", unsafe_allow_html=True)

    subset = daily_df.head(10).reset_index(drop=True)
    for start in range(0, len(subset), 5):
        cols = st.columns(5)
        for offset, (_, row) in enumerate(subset.iloc[start:start + 5].iterrows()):
            idx = start + offset
            label = format_dag_kop(row["time"])
            if cols[offset].button(label, key=f"bulletin_day_{idx}"):
                st.session_state["selected_bulletin_day_idx"] = idx

    selected_idx = int(st.session_state.get("selected_bulletin_day_idx", 0))
    selected_idx = max(0, min(selected_idx, len(subset) - 1))
    row = subset.iloc[selected_idx]
    omschrijving, icoon = weer_info(row["weather_code"])
    short, extended = build_bulletins(row, location_name)

    highlights = [
        ("Weerbeeld", f"{icoon} {omschrijving}"),
        ("Temperatuur", f"{round_int(row['temperature_2m_min'])}° / {round_int(row['temperature_2m_max'])}°"),
        ("Neerslag", f"{round_int(row['precipitation_probability_max'])}% · {row['precipitation_sum']:.1f} mm"),
        ("Wind", f"{round_int(row['wind_speed_10m_max'])} km/u · {wind_beschrijving(row['wind_speed_10m_max'])}"),
    ]
    highlight_html = "".join(
        html_block(
            f"""
    ...
    st.markdown(
        html_block(
            f"""
            <div class="radio-card">
                <div class="radio-day-title">{format_dag_kop(row['time'])}</div>
                <div class="radio-meta">Belangrijkste punten voor {location_name}: zonsopkomst {row['sunrise'].strftime('%H:%M')} · zonsondergang {row['sunset'].strftime('%H:%M')} · UV {row['uv_index_max']:.1f}</div>
                <div class="radio-highlight-grid" style="margin-top:16px;">{highlight_html}</div>
            </div>
            """
        ),
        unsafe_allow_html=True,
    ...
    st.markdown("#### Korte weerbulletins")
    st.text_area(
        "Korte weerbulletins",
        value=short,
        height=110,
        key=f"bulletin_short_{selected_idx}",
        label_visibility="collapsed",
    )
    st.caption("Geschikt voor een kort bulletin of snelle vooruitblik in je programma.")

    st.markdown("#### Lange weerbulletins")
    st.text_area(
        "Lange weerbulletins",
        value=extended,
        height=145,
        key=f"bulletin_extended_{selected_idx}",
        label_visibility="collapsed",
    )
    st.caption("Iets uitgebreider script voor een opgenomen weerblok of weekvooruitblik.")
...
def main() -> None:
    ...
    enable_hourly_autorefresh()

    try:
        selected_location = get_selected_location_from_state()
        location_name = selected_location.get("name", DEFAULT_LOCATION["name"])
        hourly_df, daily_df, current = fetch_weather(selected_location["latitude"], selected_location["longitude"])

        render_hero(current, location_name)
        ...
        with tab3:
            ...
            render_bulletin_picker(daily_df, location_name)
