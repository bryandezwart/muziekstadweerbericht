...
CSS = dedent(
    ...
    .hero, .ticker-card, .glass-card, .radio-card {
    ...
    .summary-grid, .hour-grid, .daily-grid, .radio-highlight-grid {
        display: grid;
        gap: 16px;
    }
    ...
    .radio-highlight-grid { grid-template-columns: repeat(auto-fit, minmax(190px, 1fr)); }

    .summary-card, .hour-card, .daily-card, .radio-highlight {
    ...
    .summary-value, .radio-value {
        font-size: 1.35rem;
        font-weight: 900;
        margin-bottom: 6px;
    }

    .summary-desc, .radio-copy-hint {
        font-size: 0.98rem;
        color: rgba(255,255,255,0.78);
        line-height: 1.45;
    }
    ...
    .hour-icon, .daily-icon, .radio-icon {
        font-size: 1.9rem;
        margin-bottom: 6px;
    }
    ...
    .hour-meta, .daily-meta, .radio-meta {
...
def search_locations(query: str) -> list[dict]:
    ...
    response = requests.get(
        ...
        params={"name": query, "count": 10, "language": "nl", "format": "json"},
        timeout=20,
    )
...
def build_radio_scripts(row: pd.Series, location_name: str) -> tuple[str, str]:
...
def render_radio_day_picker(daily_df: pd.DataFrame, location_name: str) -> None:
    if "selected_radio_day_idx" not in st.session_state:
        st.session_state["selected_radio_day_idx"] = 0

    st.markdown(
        html_block(
            """
            <div class="radio-card">
                <div class="section-title">Radiotekst per dag</div>
    ...
    for start in range(0, len(subset), 5):
        ...
        for offset, (_, row) in enumerate(subset.iloc[start:start + 5].iterrows()):
            ...
            if cols[offset].button(label, key=f"radio_day_{idx}"):
                st.session_state["selected_radio_day_idx"] = idx

    selected_idx = int(st.session_state.get("selected_radio_day_idx", 0))
    selected_idx = max(0, min(selected_idx, len(subset) - 1))
    row = subset.iloc[selected_idx]
    omschrijving, icoon = weer_info(row["weather_code"])
    short, extended = build_radio_scripts(row, location_name)
...
def main() -> None:
    ...
    try:
        ...
        with tab3:
            render_radio_day_picker(daily_df, location_name)
